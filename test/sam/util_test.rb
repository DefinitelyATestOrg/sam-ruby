...
class Sam::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      ...
      @io = Sam::Util::ReadIOAdapter.new(io) {}
  ...
  def test_file_encode
    ...
    cases.each do |body, val|
      encoded = Sam::Util.encode_content(headers, body)
...
class Sam::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Sam::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Sam::Util.writable_enum do |y|
...
class Sam::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Sam::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = Sam::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_external_iteration
    it = [1, 2, 3].to_enum
    first = it.next
    fused = Sam::Util.fused_enum(it, external: true)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Sam::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Sam::Util.fused_enum(enum)
    fused_2 = Sam::Util.decode_lines(fused_1)
    fused_3 = Sam::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Sam::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class Sam::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Sam::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Sam::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Sam::Util.decode_sse(input).map(&:compact)
