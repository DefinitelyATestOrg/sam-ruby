...
module SamRuby
  ...
  class BaseClient
    ...
    def initialize(
      ...
      @headers = SamRuby::Util.normalized_headers(
        {
          "X-Stainless-Lang" => "ruby",
          "X-Stainless-Package-Version" => SamRuby::VERSION,
      ...
      @base_path = SamRuby::Util.normalize_path(base_url_parsed.path)
    ...
    # @param opts [Hash{Symbol => Object}, SamRuby::RequestOptions]
    #
    # @raise [ArgumentError]
    def validate_request(req, opts)
      ...
      case opts
      in SamRuby::RequestOptions | Hash
        valid_keys = SamRuby::RequestOptions.options
    ...
    def resolve_uri_elements(req)
      from_args =
        if (url = req[:url])
          SamRuby::Util.parse_uri(url)
        else
          path = SamRuby::Util.normalize_path("/#{@base_path}/#{req.fetch(:path)}")
          req.slice(:scheme, :host, :port, :query).merge(path: path)
        end

      query = SamRuby::Util.deep_merge(
        from_args[:query] || {},
        req[:extra_query] || {},
        concat: true
      )
    ...
    def prep_request(options)
      ...
      if options[:extra_body]
        body = SamRuby::Util.deep_merge(body, options[:extra_body])
      end
      ...
      loop do # rubocop:disable Metrics/BlockLength
        ...
        begin
          ...
          if status < 300
            ...
            prev_uri = SamRuby::Util.unparse_uri(request)
            ...
            # from undici
            if SamRuby::Util.uri_origin(prev_uri) != SamRuby::Util.uri_origin(location)
              request[:headers] = request[:headers].reject do |k|
    ...
    # @param req [Hash{Symbol => Object}]
    # @param opts [SamRuby::RequestOptions, Hash{Symbol => Object}]
    #
    # @return [Object]
    def request(req, opts)
      validate_request(req, opts)
      options = SamRuby::Util.deep_merge(req, opts)
    ...
    # @return [String]
    def inspect
      base_url = SamRuby::Util.unparse_uri(scheme: @scheme, host: @host, port: @port, path: @base_path)
