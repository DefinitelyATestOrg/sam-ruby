...
module SamRuby
  module Models
    class InboundMailItem < BaseModel
      ...
      #   @return [Symbol]
      required :rejection_reason, SamRuby::Enum.new(:no_matching_lockbox, :no_check, :lockbox_not_active)

      # @!attribute [rw] return_address
      #   The return address as written on the mail item.
      #   @return [SamRuby::Models::InboundMailItem::ReturnAddress]
      required :return_address, -> { SamRuby::Models::InboundMailItem::ReturnAddress }

      # @!attribute [rw] status
      #   If the mail item has been processed.
      #   @return [Symbol]
      required :status, SamRuby::Enum.new(:pending, :processed, :rejected)

      # @!attribute [rw] type
      #   A constant representing the object's type. For this resource it will always be `inbound_mail_item`.
      #   @return [Symbol]
      required :type, SamRuby::Enum.new(:inbound_mail_item)
