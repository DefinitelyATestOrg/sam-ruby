...
module SamRuby
  module Models
    class DigitalCardProfile < BaseModel
      ...
      #   @return [Symbol]
      required :status, SamRuby::Enum.new(:pending, :rejected, :active, :archived)

      # @!attribute [rw] text_color
      #   The Card's text color, specified as an RGB triple.
      #   @return [SamRuby::Models::DigitalCardProfile::TextColor]
      required :text_color, -> { SamRuby::Models::DigitalCardProfile::TextColor }

      # @!attribute [rw] type
      #   A constant representing the object's type. For this resource it will always be `digital_card_profile`.
      #   @return [Symbol]
      required :type, SamRuby::Enum.new(:digital_card_profile)
