...
module SamRuby
  module Models
    class IntrafiAccountEnrollment < BaseModel
      ...
      #   @return [Symbol]
      required :status,
               SamRuby::Enum.new(:pending_enrolling, :enrolled, :pending_unenrolling, :unenrolled, :requires_attention)

      # @!attribute [rw] type
      #   A constant representing the object's type. For this resource it will always be `intrafi_account_enrollment`.
      #   @return [Symbol]
      required :type, SamRuby::Enum.new(:intrafi_account_enrollment)
    end
  end
end
