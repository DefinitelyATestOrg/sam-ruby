...
module SamRuby
  module Models
    class WireDrawdownRequest < BaseModel
      ...
      #   @return [Symbol]
      required :status, SamRuby::Enum.new(:pending_submission, :pending_response, :fulfilled, :refused)

      # @!attribute [rw] submission
      #   After the drawdown request is submitted to Fedwire, this will contain supplemental details.
      #   @return [SamRuby::Models::WireDrawdownRequest::Submission]
      required :submission, -> { SamRuby::Models::WireDrawdownRequest::Submission }

      # @!attribute [rw] type
      #   A constant representing the object's type. For this resource it will always be `wire_drawdown_request`.
      #   @return [Symbol]
      required :type, SamRuby::Enum.new(:wire_drawdown_request)
