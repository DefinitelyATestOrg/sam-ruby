...
module Sam
  ...
  class BaseClient
    ...
    def initialize(
      ...
      @requester = Sam::PooledNetRequester.new
      ...
      @base_url = Sam::Util.parse_uri(base_url)
    ...
    def resolve_url(req)
      ...
      req_path, req_query = Sam::Util.parse_uri(req.fetch(:path)).fetch_values(:path, :query)
      ...
      joined.query = Sam::Util.encode_query(query)
      joined
    end
    ...
    private def build_request(req, opts)
      ...
      if [:get, :head, :options].include?(method)
        ...
        body = Sam::Util.deep_merge(body, extra_body)
      end
    ...
    private def retry_delay(response, retry_count:)
      ...
      span = retry_header && Sam::Util.suppress(ArgumentError) do
    ...
    private def follow_redirect(request, url:, status:, location_header:)
      ...
      location =
        Sam::Util.suppress(ArgumentError) do
      ...
      # from undici
      if Sam::Util.uri_origin(url) != Sam::Util.uri_origin(location)
    ...
    private def parse_response(req, opts, response)
      ...
      in [nil, _] unless model.nil?
        Sam::Converter.convert(model, raw_data)
      in [nil, nil]
        raw_data
      end
    end
