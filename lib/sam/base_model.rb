...
module Sam
  ...
  module Converter
    ...
    #
    #   @option spec [Proc, Sam::Converter, Class] :enum
    #
    #   @option spec [Proc, Sam::Converter, Class] :union
    #
    ...
    def self.try_strict_coerce(target, value)
      ...
      in Class
        ...
        in [-> { _1 <= Date || _1 <= Time }, String]
          Sam::Util.suppress(ArgumentError, Date::Error) do
  ...
  class Union
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= Sam::Union && other.variants == variants
    end
  ...
  class ArrayOf
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Sam::Converter.coerce(type, item) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Sam::Converter.dump(type, item) }.to_a
      else
        value
      end
    end
  ...
  class BaseModel
    ...
    #
    # @param type_info [Proc, Sam::Converter, Class, Hash{Symbol=>Object}, nil]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, Sam::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    #
    def self.coerce(value)
      case (coerced = Sam::Util.coerce_hash(value))
      in Hash
        new(coerced)
      else
        value
    ...
    #
    def initialize(data = {})
      case (coerced = Sam::Util.coerce_hash(data))
      in Hash
