...
module Sam
  ...
  module Converter
    ...
    class << self
      # @api private
      #
      # @param spec [Hash{Symbol=>Object}, Proc, Sam::Converter, Class] .
      #
      ...
      #
      # @param target [Sam::Converter, Class]
      #
      # @param value [Object]
      #
      ...
      def dump(target, value)
        target.is_a?(Sam::Converter) ? target.dump(value) : Sam::Unknown.dump(value)
      end
    end
  end
  ...
  module Union
    ...
    # @api private
    #
    # @param key [Symbol, Hash{Symbol=>Object}, Proc, Sam::Converter, Class]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, Sam::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    private def variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Sam::Converter.type_info(spec)]
        in Proc | Sam::Converter | Class | Hash
          [nil, Sam::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [Sam::Converter, Class, nil]
    private def resolve_variant(value)
      ...
      in [Symbol, Hash]
        key = value.fetch(@discriminator) do
          value.fetch(@discriminator.to_s, Sam::Util::OMIT)
        end

        return nil if key == Sam::Util::OMIT
    ...
    def coerce(value, state:)
      if (target = resolve_variant(value))
        return Sam::Converter.coerce(target, value, state: state)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        coerced = Sam::Converter.coerce(target, value, state: state)
    ...
    def dump(value)
      if (target = resolve_variant(value))
        return Sam::Converter.dump(target, value)
      end

      known_variants.each do
        target = _2.call
        return Sam::Converter.dump(target, value) if target === value
      end
  ...
  class ArrayOf
    ...
    # @param type_info [Hash{Symbol=>Object}, Proc, Sam::Converter, Class]
    #
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Sam::ArrayOf) && other.nilable? == nilable? && other.item_type == item_type
      ...
      value
        .map do |item|
          ...
          in [true, nil]
            ...
            Sam::Converter.coerce(target, item, state: state)
          end
        end
    end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Array) ? value.map { Sam::Converter.dump(target, _1) } : super
    end

    # @api private
    #
    # @return [Sam::Converter, Class]
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = Sam::Converter.type_info(type_info || spec)
      @nilable = spec[:nil?]
    end
  end
  ...
  class HashOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Sam::HashOf) && other.nilable? == nilable? && other.item_type == item_type
        ...
        .to_h do |key, val|
          ...
          v =
            ...
            in [true, nil]
              ...
              Sam::Converter.coerce(target, val, state: state)
            end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Hash) ? value.transform_values { Sam::Converter.dump(target, _1) } : super
    end
  ...
  class BaseModel
    ...
    class << self
      ...
      def known_fields
        @known_fields ||= (self < Sam::BaseModel ? superclass.known_fields.dup : {})
      end
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | Sam::Converter | Class
            [Sam::Converter.type_info({**spec, union: type_info}), spec]
          in Hash
            [Sam::Converter.type_info(type_info), type_info]
          end

        setter = "#{name_sym}="
        api_name = info.fetch(:api_name, name_sym)
        nilable = info[:nil?]
        const = required && !nilable ? info.fetch(:const, Sam::Util::OMIT) : Sam::Util::OMIT
        ...
        define_method(name_sym) do
          target = type_fn.call
          value = @data.fetch(name_sym) { const == Sam::Util::OMIT ? nil : const }
          state = {strictness: :strong, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
          (nilable || !required) && value.nil? ? nil : Sam::Converter.coerce(target, value, state: state)
          ...
          raise Sam::ConversionError.new(message)
        end
      end
      ...
      #
      # @return [Boolean]
      def ==(other) = other.is_a?(Class) && other <= Sam::BaseModel && other.fields == fields
    end
    ...
    class << self
      # @api private
      #
      # @param value [Sam::BaseModel, Hash{Object=>Object}, Object]
      #
      # @param state [Hash{Symbol=>Object}] .
      #
      #   @option state [Boolean, :strong] :strictness
      ...
      def coerce(value, state:)
        ...
        unless (val = Sam::Util.coerce_hash(value)).is_a?(Hash)
          exactness[:no] += 1
          return value
        end
        exactness[:yes] += 1
        ...
        fields.each do |name, field|
          ...
          converted =
            if item.nil? && (nilable || !required)
              ...
              coerced = Sam::Converter.coerce(target, item, state: state)
              case target
              in Sam::Converter | Symbol
                coerced
              else
                item
              end
      ...
      # @api private
      #
      # @param value [Sam::BaseModel, Object]
      #
      # @return [Hash{Object=>Object}, Object]
      def dump(value)
        unless (coerced = Sam::Util.coerce_hash(value)).is_a?(Hash)
          return super
        end
        ...
        coerced.each do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              acc.store(api_name, Sam::Converter.dump(target, val))
            end
          end
        end
        ...
        known_fields.each_value do |field|
          ...
          next if mode == :coerce || acc.key?(api_name) || const == Sam::Util::OMIT
    ...
    def initialize(data = {})
      case Sam::Util.coerce_hash(data)
      in Hash => coerced
        @data = coerced
      else
        raise ArgumentError.new("Expected a #{Hash} or #{Sam::BaseModel}, got #{data.inspect}")
      end
    end
